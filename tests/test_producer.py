...
def test_configure_concurrent_calls_do_not_crash():
    app_one = Celery("concurrent-one", broker="redis://127.0.0.1:6379/0", backend="redis://127.0.0.1:6379/1")
    app_two = Celery("concurrent-two", broker="redis://127.0.0.1:6379/0", backend="redis://127.0.0.1:6379/1")
...
def test_send_task_default_queue_when_empty_requirements(celery_app, run_worker):
    @celery_app.task(name="tasks.ping")
    def ping() -> str:
        return "pong"

    configure(celery_app)
    with run_worker("producer-default@local", capabilities=["FactoryB", "gpu"]):
        result = send_task("tasks.ping", requirements=[])
        assert result.get(timeout=20) == "pong"


def test_send_task_default_queue_when_none_requirements(celery_app, run_worker):
    ...
    configure(celery_app)
    with run_worker("producer-none@local", capabilities=[]):
        result = send_task("tasks.ping_none", requirements=None)
        assert result.get(timeout=20) == "pong-none"


def test_send_task_simple_requirement_routes_directly(celery_app, run_worker):
    @celery_app.task(name="tasks.inference")
    def inference() -> str:
        return "gpu-ok"

    configure(celery_app)
    with run_worker("producer-gpu@local", capabilities=["gpu"]):
        result = send_task("tasks.inference", requirements=["gpu"])
        assert result.get(timeout=20) == "gpu-ok"


def test_send_task_simple_requirement_normalizes_input(celery_app, run_worker):
    ...
    configure(celery_app)
    with run_worker("producer-normalize@local", capabilities=["gpu"]):
        result = send_task("tasks.inference_normalize", requirements=["  gpu  "])
        assert result.get(timeout=20) == "normalized"


def test_send_task_complex_requirement_triggers_awakening(celery_app, run_worker):
...
def test_send_task_with_args_and_kwargs(celery_app, run_worker):
    ...
    configure(celery_app)
    with run_worker("producer-args@local", capabilities=[]):
        result = send_task(
...
def test_producer_task_without_task_spec(celery_app, run_worker):
    ...
    with run_worker("producer-simple@local", capabilities=[]):
        result = simple_task.delay(5)
        assert result.get(timeout=20) == 10


def test_producer_task_delay_routes_to_default_queue_without_requirements(celery_app, run_worker):
    ...
    with run_worker("producer-delay@FactoryB", capabilities=[]):
        result = render.delay("frame-42")
        assert result.get(timeout=20) == "frame-42"


def test_producer_task_delay_routes_using_requirements_kwarg(celery_app, run_worker):
    producer = Producer(celery_app)
    ...
    with run_worker("producer-kwargs@gpu", capabilities=["gpu"]):
        result = add.delay(4, 4, CSR=["gpu"])
        assert result.get(timeout=20) == 8


def test_producer_task_delay_requirements_kwarg_triggers_complex_awakening(celery_app, run_worker):
    register_awakening_task(celery_app, inspect_timeout=1.5)
    producer = Producer(celery_app)
    ...
    with run_worker(
        "producer-kwargs-complex@FactoryB",
        capabilities=["FactoryB", "gpu"],
        pool="threads",
        concurrency=2,
    ):
...
def test_producer_task_apply_async_with_explicit_queue_bypasses_routing(celery_app, run_worker):
    ...
    with run_worker("producer-explicit@local", capabilities=["custom_queue"]):
...
def test_producer_task_apply_async_with_csr_in_kwargs(celery_app, run_worker):
    ...
    with run_worker("producer-async-csr@local", capabilities=["gpu"]):
        result = compute.apply_async(args=(5,), kwargs={CSR: ["gpu"]})
        assert result.get(timeout=20) == 15


def test_producer_task_csr_not_passed_to_function(celery_app, run_worker):
    ...
    with run_worker("producer-strip@local", capabilities=["gpu"]):
...
def test_producer_task_with_task_spec_and_csr_combined(celery_app, run_worker):
    ...
    with run_worker("producer-combined@local", capabilities=["arm"]):
        result = specialized_compute.delay(3, CSR=["arm"])
        assert result.get(timeout=20) == 12


def test_producer_task_csr_normalizes_requirements(celery_app, run_worker):
    ...
    with run_worker("producer-csr-norm@local", capabilities=["gpu"]):
        result = task_norm.delay(42, CSR=["  gpu  "])
        assert result.get(timeout=20) == 42


def test_producer_task_csr_empty_list_routes_to_default(celery_app, run_worker):
    ...
    with run_worker("producer-csr-empty@local", capabilities=[]):
        result = task_empty.delay(10, CSR=[])
        assert result.get(timeout=20) == 11


def test_producer_task_multiple_csr_requirements_triggers_awakening(celery_app, run_worker):
...
def test_producer_task_apply_async_with_other_options(celery_app, run_worker):
    ...
    with run_worker("producer-opts@local", capabilities=[]):
...
def test_send_task_queue_name_deterministic(celery_app, run_worker):
...
def test_send_task_routes_to_configured_default_queue(celery_app, run_worker):
    ...
    configure(celery_app)
    with run_worker("producer-primary@local", capabilities=[]):
        result = send_task("tasks.ping_primary", requirements=[])
        assert result.get(timeout=20) == "pong-primary"


def test_send_task_complex_requirements_use_configured_default_queue_for_awakening(celery_app, run_worker):
    celery_app.conf.task_default_queue = "primary"
    register_awakening_task(celery_app, inspect_timeout=1.5)
    ...
    configure(celery_app)
    with run_worker(
        "producer-complex-primary@FactoryB",
        capabilities=["FactoryB", "gpu"],
        pool="threads",
        concurrency=2,
    ):
...
def test_send_task_complex_requirements_use_configured_default_queue_for_awakening_not_in_explicit_queues(celery_app, run_worker):
